...
function createAppAPI(render) {
    return function createApp(rootComponent) {
        const app = {
            ...
            mount(rootContainer) {
                console.log("基于根组件创建 vnode---build",rootContainer);
                const vnode = createVNode(rootComponent);
                console.log("调用 render，基于 vnode 进行开箱");
                console.log(vnode, rootContainer, vnode, rootContainer);
...
function createRenderer(options) {
    ...
    function setupRenderEffect(instance, initialVNode, container) {
        console.log(instance, initialVNode, container,'instance, initialVNode, container');
...
function parseChildren(context, ancestors) {
    console.log("开始解析 children", context, JSON.stringify(ancestors));
    const nodes = [];
    while (!isEnd(context, ancestors)) {
        ...
        if (startsWith(s, "{{")) {
            ...
            console.log(JSON.stringify(node),'node  解析到的的node--------');
        }
        else if (s[0] === "<") {
            if (s[1] === "/") {
                if (/[a-z]/i.test(s[2])) {
                ...
                console.log(node,'node  解析到的的children--------');
            }
        }
        if (!node) {
            node = parseText(context);
            console.log(JSON.stringify(node),'node  解析到的的变量--------');
...
function parseElement(context, ancestors) {
    ...
    console.log(element,'解析了element');
    ancestors.push(element);
    const children = parseChildren(context, ancestors);
    console.log(JSON.stringify(children),'children')
    console.log(JSON.stringify(ancestors),'ancestors')
    ancestors.pop();
    console.log(JSON.stringify(ancestors),'ancestors')
    console.log(JSON.stringify(context.source),'context.source')
...
function parseInterpolation(context) {
    console.log(context,'推进代码 context')
    const openDelimiter = "{{";
    const closeDelimiter = "}}";
    const closeIndex = context.source.indexOf(closeDelimiter, openDelimiter.length);
    console.log(closeIndex,'closeIndex');
    advanceBy(context, 2);
    console.log(context,'推进代码d dddddddddddd context')
    ...
    console.log(preTrimContent,'preTrimContent-------------');
    const content = preTrimContent.trim();
    console.log(content,'content')
    console.log(context,'推进代码 删除变量')

    advanceBy(context, closeDelimiter.length);
    console.log(context,'推进代码 删除}}')
    return {
        type: 2,
        content: {
            type: 3,
...
function parseTextData(context, length) {
    console.log("解析 textData",context, length);
...
function startsWith(source, searchString) {
    ...
    console.log(source.startsWith(searchString),'source.startsWith(searchString)');
